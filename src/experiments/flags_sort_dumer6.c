...
void sub_isd_init(isd_params* params, word* local_L, word* local_synds, unsigned int local_N, sw_list** local_h, ranctx* state) {
	...
	trunc_xors_table = (word*) MALLOC(nb_of_sums * sizeof(word));
...
/* Intermediate sums */
	...
	*/
	long long nb_of_sums = nCr(L_len/2, P/2);
	for (c1 = 2; c1 < L_len/2; ++c1) {
		sum1 = L[c1];
	for (c2 = 1; c2 < c1; ++c2) {
		sum12 = sum1 ^ L[c2];
	...
	for (c = 0; c < nb_of_sums; ++c) {
		trunc_xors_table[c] = xors_table[c] >> shift;
	}
	for (c = 0; c < nb_of_sums; ++c) {
		counterht_store(L0, trunc_xors_table[c], c);
	}
	for (c = 0; c < nb_of_sums; ++c) {
		index = trunc_xors_table[c];
		L0_flags[index / (8*sizeof(uint64_t))] |= 1UL << (index % (8*sizeof(uint64_t)));
	}
