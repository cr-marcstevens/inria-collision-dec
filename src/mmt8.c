...
void sub_isd() {
	...
	for (c1 = 1; c1 < L_len/2; ++c1) {
		for (c2 = 0; c2 < c1; ++c2) {
			...
			iaht_store(L1_indices, index, p/4, c1, c2);
		}
	}

	// L2, L4
	for (c1 = (L_len+1)/2 + 1; c1 < L_len; ++c1) {
		for (c2 = (L_len+1)/2; c2 < c1; ++c2) {
			value = L[c1] ^ L[c2];
			index = value & l2mask;
			waht_store(L2, index, value);
			iaht_store(L2_indices, index, p/4, c1, c2);
		}
	}
										...
										for (i2 = 1, p2 = L2_indices[a ^ old_x]+1; i2 < 1 + L2_indices[a ^ old_x][0]; ++i2, p2 += p/4) {
											for (i3 = 1, p3 = L3_indices[x]+1; i3 < 1 + L3_indices[x][0]; ++i3, p3 += p/4) {
												for (i4 = 1, p4 = L4_indices[aprime ^ x]+1; i4 < 1 + L4_indices[aprime ^ x][0]; ++i4, p4 += p/4) {
													...
													c5 = p3[0]; c6 = p3[1];
													c7 = p4[0]; c8 = p4[1];
													weight_on_one_word = isd_weight(L[c1] ^ L[c2] ^ L[c3] ^ L[c4] ^ L[c5] ^ L[c6] ^ L[c7] ^ L[c8] ^ synd);
													if(weight_on_one_word <= threshold) {
														incr_final_test_counter();
														final_test_probe_start();
														final_weight = final_test(0, weight_on_one_word, p, c1, c2, c3, c4, c5, c6, c7, c8);
														final_test_probe_stop();

														if (final_weight != -1) {
															*h = sw_list_add(*h, 0, final_weight, p, c1, c2, c3, c4, c5, c6, c7, c8);
														}
													}
												}
											}
